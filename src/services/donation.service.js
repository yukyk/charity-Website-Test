...
/**
 * Validates that the charity is approved and (if supplied) the project
 * belongs to that charity and is active. Returns both objects.
 */
...
/**
 * Atomically marks a donation as completed and increments raisedAmount on
 * the charity (and project if applicable) inside one DB transaction.
 * Using Sequelize increment() generates "SET raisedAmount = raisedAmount + ?"
 * which is safe under concurrent writes.
 */
...
/**
 * Fires in-app notification + confirmation email after a successful donation.
 * Non-blocking — errors are logged, never re-thrown.
 */
